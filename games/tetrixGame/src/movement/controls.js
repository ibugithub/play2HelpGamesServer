...
export class Controls {
  ...
  onKeyDown(event, key) {
    ...
    if (
      this.game.modals.open ||
      this.game.modals.closing ||
      this.game.mechanics.locking.timings.clearDelay != 0
    )
      return;
    if (event.key != this.menuKey && !this.game.started)
      this.moves.startTimers();
  ...
  onKeyDownRepeat(event, key) {
    // allows for arr undo/redo
    if (this.game.replay.state == "replaying") return;
  ...
  timer() {
    if (this.currentDirection == undefined || this.startedDas == undefined)
      return;
  ...
  startArr(direction) {
    if (direction == "current") {
      if (
        this.directionState["RIGHT"] == "arr" &&
        this.directionState["LEFT"] == "arr"
      )
        return;
